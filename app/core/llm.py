...
DEFAULT_GENERATION_MODEL = os.getenv("HUGGINGFACE_MODEL", "Qwen/Qwen2.5-0.5B-Instruct")
...
def _build_messages(query: str, context: str) -> list[dict]:
    system_prompt = (
        "Eres el chatbot oficial de Latinoamérica Comparte, "
        "una red presente en Colombia, Ecuador, Chile y Argentina. "
...
_tokenizer, _model = load_generation_model(DEFAULT_GENERATION_MODEL)


def generate(prompt: str, context: str = "") -> str:
    return generate_answer(prompt, context, _tokenizer, _model)
